/*
 * Audio Clip Player
 * TCSS 305 - Autumn 2008 (Zimmerman)
 */
package tetris.gui;
...
public class SoundPlayer 
{
   ...
   * playing.
   * 
   * @param the_filename The name of the file (relative to the current
  ...
  /** 
   * Plays the audio file with the given file name, waiting until 
   * the clip is done playing before returning.
   * 
   * @param the_filename The name of the file (relative to the current
   * working directory) to play.
   * @return a Clip object representing the sound played.
  ...
  public Clip playAndWait(final String the_filename) 
    throws IllegalArgumentException
  {
    final Clip clip = getClip(the_filename);
  ...
  /** 
   * Plays the clip with the given file name in a loop.
   * The clip loops continuously until it is stopped by calling
   * the stop() method. This method waits until the clip is done 
   * looping before returning. Note that since the clip loops 
   * continuously, this method will not return unless some other 
  ...
  public Clip loopAndWait(final String the_filename) 
    throws IllegalArgumentException
  {
   ...
   * Plays the clip with the given file name in a loop.
   * The clip loops until it has played the specified number of 
   * times, or until it is stopped by calling the stop() method. 
