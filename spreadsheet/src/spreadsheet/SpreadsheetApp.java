 ...
 * 
 * @author Donald Chinn modified by Teddy Doll
 **/

public class SpreadsheetApp {

	/**
	 * Read a string from standard input. All characters up to the first
	 * carriage return are read. The return string does not include the carriage
	 * return.
	 * 
	...
	private static void menuPrintCellFormula(Spreadsheet theSpreadsheet) {
		...
		Cell.getCellToken(inputString, 0, cellToken);// modified code to call
														// Cell static
														// method(Teddy)

		System.out.println(theSpreadsheet.cellValueToString(cellToken));
		System.out.println(": ");
	...
	private static void menuChangeCellFormula(Spreadsheet theSpreadsheet) {
		...
		try {
			theSpreadsheet.changeCellFormulaAndRecalculate(cellToken,
					inputFormula);
		} catch (CycleFound e) {
		...
		System.out.println();
		/*
		 * // This code prints out the expression stack from // top to bottom
		 * (that is, reverse of postfix). while (!expTreeTokenStack.isEmpty()) {
	...
	public static void main(String[] args) {
		Spreadsheet theSpreadsheet = new Spreadsheet(2);
		...
		while (!done) {
			...
			/*
			 * BONUS: case 'r': menuReadSpreadsheet(theSpreadsheet); break;
			 * 
			 * case 's': menuSaveSpreadsheet(theSpreadsheet); break;
			 */
